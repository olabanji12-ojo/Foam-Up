...
const CarwashDashboard = () => {
  ...
  const updateBookingStatus = async (bookingId, newStatus) => {
    try {
      ...
      if (activeTab === "bookings") {
        ...
        // Refresh dashboard data if boon dashboard tab
  ...
  const fetchSettings = async () => {
    try {
      ...
      // Map all fields properly, including open_hours
      setSettings({
        ...
        // ✅ Properly handle open_hours, even if some days are missing in API response
        open_hours: {
          mon: {
            start: res.data.data.open_hours?.mon?.start || "",
            end: res.data.data.open_hours?.mon?.end || "",
          },
          tue: {
            start: res.data.data.open_hours?.tue?.start || "",
            end: res.data.data.open_hours?.tue?.end || "",
          },
          wed: {
            start: res.data.data.open_hours?.wed?.start || "",
            end: res.data.data.open_hours?.wed?.end || "",
          },
          thu: {
            start: res.data.data.open_hours?.thu?.start || "",
            end: res.data.data.open_hours?.thu?.end || "",
          },
          fri: {
            start: res.data.data.open_hours?.fri?.start || "",
            end: res.data.data.open_hours?.fri?.end || "",
          },
          sat: {
            start: res.data.data.open_hours?.sat?.start || "",
            end: res.data.data.open_hours?.sat?.end || "",
          },
          sun: {
            start: res.data.data.open_hours?.sun?.start || "",
            end: res.data.data.open_hours?.sun?.end || "",
          },
        },
      });
    } catch (err) {
      ...
      <div className="container mx-auto p-4">
        ...
        <nav className="mb-4">
          <ul className="flex space-x-4 border-b">
            {["dashboard", "bookings", "services", "settings"].map(
              (tab) => (
                <li key={tab}>
                  <button
                    className={`p-2 ${
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="bg-white p-4 rounded shadow">
                    ...
                    <p className="text-2xl">₦{Number(dashboardData.earnings || 0).toFixed(2)}</p>
                  </div>
                  <div className="bg-white p-4 rounded shadow">
                    <h3 className="text-lg font-semibold">Today's Bookings</h3>
                    <p className="text-2xl">{dashboardData.bookings.length || 0}</p>
                  </div>
                  <div className="bg-white p-4 rounded shadow">
                ...
                {(() => {
                  ...
                  console.log("dashboardData.bookings:", dashboardData.bookings);
                  console.log("dashboardData.bookings.length:", dashboardData.bookings.length);
                  console.log("Condition (length === 0):", dashboardData.bookings.length === 0);
                  return null;
                })()}
                
                {dashboardData.bookings.length === 0 ? (
                  <p className="text-center text-gray-500">No bookings for {slotDate}</p>
                ) : (
                      ...
                      {dashboardData.bookings.map((booking, index) => {
                        ...
                        return (
                          <tr key={index}>
                            ...
                            <td className="border p-2">{booking.service_name || "N/A"}</td>
                ...
                {/* Services Section */}
                <h3 className="text-lg font-semibold mt-6 mb-2">Available Services</h3>
                {dashboardData.services.length === 0 ? (
                  <p className="text-center text-gray-500">No services available</p>
                ) : (
                  <ul className="list-disc pl-6">
                    {dashboardData.services.map((service, index) => {
                      console.log(`🛠️ SERVICE ${index}:`, service);
                      return (
        ...
        {activeTab === "bookings" && (
          <div>
            ...
            {loading && <p className="text-center text-blue-500">Loading bookings...</p>}
            {error && <p className="text-center text-red-500">{error}</p>}
                    ...
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                         ...
                         {/* this was the issue is */}
                  <tbody className="bg-white divide-y divide-gray-200">
                    {Array.isArray(allBookings?.data?.data) && allBookings.data.data.length > 0 ? (
                      allBookings.data.data.map((booking, index) => (
        ...
        {activeTab === "services" && (
          <div>
            ...
            {loading && <p className="text-center text-blue-500">Loading services...</p>}
            {error && <p className="text-center text-red-500">{error}</p>}
                  ...
                  <table className="w-full">
                    ...
                    {(Array.isArray(services) ? services : []).map((service, index) => (
                        <tr key={service.id || index} className="hover:bg-gray-50">
                          <td className="px-4 py-4 whitespace-nowrap font-medium">{service.name}</td>
                          <td className="px-4 py-4">{service.description || 'N/A'}</td>
                          <td className="px-4 py-4 whitespace-nowrap">₦{service.price}</td>
                          <td className="px-4 py-4 whitespace-nowrap">{service.duration || 'N/A'} mins</td>
                          <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        className="mr-2"
                      />
                      <label htmlFor="is_active" className="text-gray-700">Service is active</label>
                    </div>
                  </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* LGA */}
            <div>
              <label className="block text-gray-700 mb-1">LGA</label>
              <input
                type="text"
                value={settings.lga || ""}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="auto_confirm" className="text-gray-700">
            ...
            <div>
              <label className="block text-gray-700 mb-1">
                Advance Booking Days (How many days in advance can customers book?)
              </label>
              <input
                type="number"
                min="1"
                ...
                onChange={(e) =>
                  setSettings({ ...settings, advance_booking_days: parseInt(e.target.value) })
                }
            ...
            {/* Max Cars Per Slot */}
            <div>
              <label className="block text-gray-700 mb-1">Max Cars Per Slot</label>
              <input
                type="number"
                value={settings.max_cars_per_slot || 1}
                onChange={(e) =>
                  setSettings({ ...settings, max_cars_per_slot: parseInt(e.target.value) })
                }
                className="w-32 p-2 border rounded"
              />
            </div>
            ...
            {/* Service Range */}
            <div>
              <label className="block text-gray-700 mb-1">Service Range (Minutes)</label>
              <input
                type="number"
                value={settings.service_range_minutes || 30}
                onChange={(e) =>
                  setSettings({ ...settings, service_range_minutes: parseInt(e.target.value) })
                }
                className="w-32 p-2 border rounded"
              />
            </div>
            ...
            {/* Delivery Radius */}
            <div>
              <label className="block text-gray-700 mb-1">Delivery Radius (KM)</label>
              <input
                type="number"
                value={settings.delivery_radius_km || 0}
                onChange={(e) =>
                  setSettings({ ...settings, delivery_radius_km: parseInt(e.target.value) })
                }
                className="w-32 p-2 border rounded"
              />
            </div>
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  setSettings({ ...settings, home_service: e.target.checked })
                }
                className="mr-2"
              />
              <label htmlFor="home_service" className="text-gray-700">
                Offer Home Service
              </label>
            </div>
          </div>
        ...
        <div className="bg-gray-50 p-4 rounded">
          ...
          <ul className="text-sm space-y-1 text-gray-600">
            ...
            <li>
              Auto-confirm Bookings:{" "}
              <span className="font-medium">
                {settings.auto_confirm_bookings ? "Yes" : "No"}
              </span>
            </li>
            <li>
              Advance Booking:{" "}
            ...
            <li>
              Max Cars Per Slot:{" "}
              <span className="font-medium">{settings.max_cars_per_slot}</span>
            </li>
            <li>
              Service Range:{" "}
              <span className="font-medium">{settings.service_range_minutes} mins</span>
            </li>
            <li>
              Delivery Radius:{" "}
              <span className="font-medium">{settings.delivery_radius_km} km</span>
            </li>
            <li>
              Home Service:{" "}
              <span className="font-medium">{settings.home_service ? "Yes" : "No"}</span>
            </li>
          </ul>
        </div>
      </div>
